...
class Dataset(torchDataset):
    ...
    def __init__(self, input_dimension, enable_aug=True):
        super().__init__()
    ...
    def wrap_getitem(getitem_fn):
        ...
        Example:
            ...
            ...     def __len__(self):
            ...         return 10
            ...     @ln.data.Dataset.wrap_getitem
            ...     def __getitem__(self, index):
            ...         return self.enable_aug
        """

        @wraps(getitem_fn)
        def wrapper(self, index):
            if not isinstance(index, int):
                self.enable_aug = index[0]
                index = index[1]
...
class CacheDataset(Dataset, metaclass=ABCMeta):
    """ This class is a subclass of the base :class:`yoloxe.data.datasets.Dataset`,
    ...
    Args:
        ...
        cache (bool): whether to cache the images to disk.
    """
    ...
    def cache_images(
        ...
        assert (data_dir and cache_dir_name and path_filename) is not None, \
            "data_dir, cache_name and path_filename must be specified"
        self.path_filename = path_filename
        ...
        if self.cache:
            ...
            for i, x in pbar:   # x = self.read_img(self, i, use_cache=False)
                ...
                b += x.nbytes
                pbar.desc = \
                    f'Caching images ({b / gb:.1f}/{mem_required / gb:.1f}GB)'
            pbar.close()
