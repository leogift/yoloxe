...
def bboxes_iou(bboxes_a, bboxes_b, xyxy=True):
    ...
    if xyxy:
        inter_tl = torch.max(bboxes_a[:, None, :2], bboxes_b[:, :2])
    ...
    diff = (w_d**2 + h_d**2) / (w_u**2 + h_u**2).clamp(1e-7)
...
def matrix_iou(a, b):
    ...
    return area_i / np.clip(area_u, 1e-12)


def adjust_points_anns(points, scale_ratio, padw, padh):
    points[:, 0::2] = points[:, 0::2] * scale_ratio + padw
    points[:, 1::2] = points[:, 1::2] * scale_ratio + padh
    return points
